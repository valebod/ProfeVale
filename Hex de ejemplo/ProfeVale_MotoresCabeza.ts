/**
 * PROFE VALE - CONTROL DE CABEZA ROBÓTICA CON MICRO:BIT
 * =====================================================
 * 
 * Este programa permite controlar una cabeza robótica mediante reconocimiento facial
 * usando la app web de Profe Vale y conexión Bluetooth.
 * 
 * HARDWARE REQUERIDO:
 * - micro:bit v2 (recomendado) o v1.5
 * - Placa de expansión con controlador de servomotores
 * - 5 servomotores SG90 o similares
 * - Fuente de alimentación externa para servos (recomendado)
 * 
 * CONEXIONES DE SERVOS (usando placa con motor driver):
 ...
 * 
 * NOTA IMPORTANTE:
 * Este código está diseñado para placas de expansión con motor driver integrado.
 * Para otras placas como WUKONG, las conexiones y comandos pueden variar.
 * 
 * COMPATIBILIDAD:
 * ✅ Placas con motor driver integrado
 * ⚠️  Para WUKONG u otras placas: requiere adaptación del código
 * 
 * FUNCIONAMIENTO:
...
bluetooth.onUartDataReceived(serial.delimiters(Delimiters.NewLine), function () {
    ...
    // Control de servomotores según valores recibidos
    // IMPORTANTE: Estos comandos son específicos para motor driver integrado
    
    // Servo boca - mapea apertura de boca (0-90° a 0-50° servo)
    motor.servo(motor.Servos.S3, Math.map(valorBoca, 0, 90, 0, 50))
    
    // Servo cabeza arriba/abajo - mapea pitch (10-90° a 180-0° servo)
    motor.servo(motor.Servos.S1, Math.map(giroVertical, 10, 90, 180, 0))
    
    // Servo cabeza izquierda/derecha - mapea yaw (10-90° a 135-45° servo)
    motor.servo(motor.Servos.S2, Math.map(giroHorizontal, 10, 90, 135, 45))
    
    // Servo ojo izquierdo - mapea parpadeo (10-60° a 135-33° servo)
    motor.servo(motor.Servos.S4, Math.map(valorOjoIzquierdo, 10, 60, 135, 33))
    
    // Servo ojo derecho - mapea parpadeo (10-60° a 0-98° servo)
    motor.servo(motor.Servos.S5, Math.map(valorOjoDerecho, 10, 60, 0, 98))
})
